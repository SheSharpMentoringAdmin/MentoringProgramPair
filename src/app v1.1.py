...
csv_file_path_raw = os.path.join(
    'data', '/Users/alyssapausanos/Library/Mobile Documents/com~apple~CloudDocs/mentorshipProject/backend/src/data/trainingData.csv')
...
df_raw.to_csv('/Users/alyssapausanos/Library/Mobile Documents/com~apple~CloudDocs/mentorshipProject/backend/src/data/trainingDataNew.csv', index=False)
...
csv_file_path = os.path.join(
    'data', '/Users/alyssapausanos/Library/Mobile Documents/com~apple~CloudDocs/mentorshipProject/backend/src/data/trainingDataNew.csv')
...
max_length = max(df_new.map(len).max())
...
def load_labels_cosine(df: pd.DataFrame):
...
selected_columns = [
    k for k in padded_df_train.columns if k.startswith('token')]
df_new_selected_column = padded_df_train[selected_columns]
...
selected_columns_skills_goals = [k for k in padded_df_train.columns if any(
    word in k.lower() for word in ['skill', 'goal'])]
df_new_skills_goals = padded_df_train[selected_columns_skills_goals]
...
selected_columns_personality = [k for k in padded_df_train.columns if any(
    word in k.lower() for word in ['personality', 'Personality'])]
df_new_personality = padded_df_train[selected_columns_personality]
...
def train_model(skills_goals, personality, preferences, labels_cosine, num_skills_goals_features, num_personality_features, num_preference_features):
    ...
    model = create_cnn_model_with_two_labels(
        num_skills_goals_features, num_personality_features, num_preference_features)
...
def train_model(skills_goals, personality, preferences, labels_cosine, num_skills_goals_features, num_personality_features, num_preference_features):
    ...
    # Train the model with both labels
    history: Model = model.fit([preferences, skills_goals, personality],
                               {"cosine_similarity": labels_cosine},
                               epochs=15, batch_size=32, validation_split=0.2)
...
history, train_cosine_mae, val_cosine_mae, model = train_model(
    skills_goals, personality, preferences, labels_cosine, skills_goals_features, personality_features, preferences_features)
...
# Save the model
model.save("/Users/alyssapausanos/Library/Mobile Documents/com~apple~CloudDocs/mentorshipProject/backend/src/mentor_recommendation_model_with_personalities_v2.1.h5")
...
model: Model = keras.models.load_model(
    "/Users/alyssapausanos/Library/Mobile Documents/com~apple~CloudDocs/mentorshipProject/backend/src/mentor_recommendation_model_with_personalities_v2.1.h5")
